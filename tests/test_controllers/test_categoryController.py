...
class CategoryControllerTestCase(TestCase):
    ...
    def setUp(self) -> None:
        ...
        res = self.client.post(
            url_for("api.Auth_login_controller"),
            json={"email": self.userAdmin.email, "password": "password"},
        )
        self.headersAdmin = {
            "Content-Type": "application/json",
            "Authorization": f'Bearer {res.json["tokens"]["access"]}',
        ...
        res = self.client.post(
            url_for("api.Auth_login_controller"),
            json={"email": self.userStaff.email, "password": "password"},
        )
        self.headersStaff = {
            "Content-Type": "application/json",
            "Authorization": f'Bearer {res.json["tokens"]["access"]}',
    ...
    def test_controller_category_retrieve_category_ok(self) -> None:
        response = self.client.get(
            url_for(
                "api.Category_retrieve_update_delete_category_controller",
                publicId=self.category.publicId,
            ),
            headers=self.headersAdmin,
        )
