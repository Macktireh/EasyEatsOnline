...
class AddToCart(Resource):
    ...
    @api.response(status.HTTP_200_OK, 'product has been successfully added to the cart.')
    @api.doc('add_product_to_cart')
    @jwt_required()
    def post(self, productPublicId: str):
        """add product to cart"""
        identity = get_jwt_identity()
        return CartServices.addProductToCart(userPublicId=identity["publicId"], productPublicId=productPublicId)
...
class RetrieveCart(Resource):
    ...
    def delete(self, productPublicId: str):
        ...
        identity = get_jwt_identity()
        return CartServices.deleteOrderToCart(userPublicId=identity["publicId"], productPublicId=productPublicId)